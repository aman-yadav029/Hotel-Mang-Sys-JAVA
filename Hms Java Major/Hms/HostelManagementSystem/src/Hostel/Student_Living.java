...
public class Student_Living implements ActionListener, KeyListener {
    JFrame f1;
    ...
    public Student_Living() {
        ...
        headtext = new JLabel("Student Details");
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == search) {
            ...
            if (aadhar.isEmpty() && bl=="<None>")
            {
                ...
                try {
                    con = DBConnection.createDBConnection();
                    ps = con.prepareStatement("select * from student where Aadhar=? and Block=?");
                    ps.setString(1, aadhar);
                    ps.setString(2, bl);
                    rs = ps.executeQuery();
                    tabledata.setModel(DbUtils.resultSetToTableModel(rs));
